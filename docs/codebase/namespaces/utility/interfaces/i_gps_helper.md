# Gps Helper Interface

- **Interface**: `i_gps_helper`
- **Namespace**: `acs::utility`
- **Include**: `#include "utility/interfaces/i_gps_helper.h"`

## Overview

Interface for reusable GPS/trajectory helper math used by navigation and steering components.
...
```mermaid
graph LR
    i_gps_helper["i_gps_helper"]
    i_gps_helper["i_gps_helper"] --> i_object["i_object"]
```
...
```cpp
[[nodiscard]] virtual double average_heading(const std::vector<cv::Vec2d>& points) const = 0;
```
...
```cpp
[[nodiscard]] virtual int pp_angle_to_servo(double degree, double min_deg, double max_deg, int min_cmd, int max_cmd) const = 0;
```
Maps a steering angle in degrees to a bounded servo command range using configured limits.
...
```cpp
[[nodiscard]] virtual double distance_between_points(const cv::Vec2d& p1, const cv::Vec2d& p2) const = 0;
```
...
```cpp
[[nodiscard]] virtual std::pair<cv::Vec2d, cv::Vec2d> get_closest_and_lookahead_points(const std::vector<cv::Vec2d>& trajectory,
                                                                                       const cv::Vec2d& current_position,
                                                                                       double lookahead_distance) const = 0;
```
Finds the closest trajectory point and a forward lookahead target point for pure-pursuit logic.
...
```cpp
[[nodiscard]] virtual std::vector<cv::Vec2d> offset_track(const std::vector<cv::Vec2d>& ref_pts,
                                                          double offset_metres,
                                                          double heading_rad) const = 0;
```
